...
class Zend_Locale_Data
{
    ...
    private static function _merge(&$to, &$from) {
        $from_t = $from->xpath('//localeDisplayNames/territories/territory');
        foreach ($from_t as $from_t_child) {
            $_attributes = $from_t_child->attributes();
            $type = null;
            ...
            if ($type === null) throw new Zend_Locale_Exception("Territory with no type attribute");
            $xpath = '//localeDisplayNames/territories/territory[@type=\''.$type.'\']';
            $to_t = $to->xpath($xpath);
            if (!$to_t) {
                if (!isset($to_territories))
                    $to_territories = $to->xpath('//localeDisplayNames/territories');
                if (1 != count($to_territories)) 
                    throw new Zend_Locale_Exception("Only one territories tag allowed");
                $new_to = $to_territories[0]->addChild('territory', (string)$from_t_child);
                foreach ($_attributes as $k => $v)
                  $new_to->addAttribute($k, $v);
            }
        }
    }
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (file_exists($filename)) {
                ...
                //Data_Icrc_Helper_Debug::msgdump('before: ', self::$_ldml[(string) $locale]->localeDisplayNames->territories);
                # Merge it !
                self::_merge(self::$_ldml[(string) $locale], $tmp);
                unset($tmp);
                //Data_Icrc_Helper_Debug::msgdump('after: ', self::$_ldml[(string) $locale]->localeDisplayNames->territories);
            }
        }
