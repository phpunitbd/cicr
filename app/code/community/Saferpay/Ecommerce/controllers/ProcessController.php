...
class Saferpay_Ecommerce_ProcessController extends Mage_Core_Controller_Front_Action
{
	...
	public function _processPayment($event){
        ...
        /** @var $order Mage_Sales_Model_Order */
		$order = Mage::getModel('sales/order');
		$lIncrementId=$this->getRequest()->getParam('id', '');
		$sqlCon = $order->getResource()->getReadConnection();
		$sqlCon->beginTransaction();
		$sqlCon->query("update sales_flat_order set status=status where increment_id=$lIncrementId");
			...
			if($order->getState() == 'pending_payment'){
				...
				if ($this->getRequest()->getParam('capture', '') == Mage_Payment_Model_Method_Abstract::ACTION_AUTHORIZE_CAPTURE){
					...
					if ($status == 'OK' && is_array($params) && isset($params['RESULT']) && $params['RESULT'] == 0){
						...
						//decode xml idp tag in get request DATA parameter
						$dataparts=explode('&',$this->getRequest()->getParam('DATA', ''));
						$idptag=urldecode($dataparts[0]);
						$p=xml_parser_create();
						xml_parse_into_struct($p, $idptag, $idpvals, $idpindex);
						xml_parser_free($p);

						$order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true, $this->__('Captured by SaferPay. Transaction ID: '.$idpvals[0]['attributes']['PROVIDERNAME'].'/'.$ret['ID']));
						if (!$order->canInvoice()) {
